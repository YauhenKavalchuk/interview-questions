<h3>
  <img src="../assets/HTML.png" width="16" height="16" />
  <span>HTML:</span>
</h3>

- [Что такое HTML и для чего он используется?](https://youtu.be/CjdCxxqObaM?t=39)
- [Что такое HTML-элемент? Какова его структура?](https://youtu.be/ia3VPHMESnY?t=34)
- [Что такое атрибуты в HTML? Как они используются в элементах?](https://youtu.be/ia3VPHMESnY?t=102)
- [Какие глобальные атрибуты есть в HTML?](https://youtu.be/trriSYNrHw4?t=360)
- [Что такое doctype? И для чего он используется?](https://youtu.be/ycYp7CYOnO0?t=65)
- [Опишите базовую структуру HTML-страницы?](https://youtu.be/ycYp7CYOnO0?t=103)
- [Если представить HTML5 как открытую веб-платформу, из каких блоков он состоит?](https://youtu.be/yvOXvZ8aEFo?t=28)
- [Что такое валидация? И какие типы проверок HTML документа вы знаете?](https://youtu.be/G7hLwudGWL4?t=39)
- [Основные этапы проверок валидности HTML-документа?](https://youtu.be/7TvS0iKR3_c?t=133)
- [Что такое семантика? Какие семантические тэги вы знаете?](https://youtu.be/ycYp7CYOnO0?t=149)
- [Почему стоит использовать семантические тэги в верстке?](https://youtu.be/3NGkctg4lsE?t=229)
- [Какие проблемы могут возникать при использовании `<div>` вместо семантических элементов?](https://youtu.be/1kzbNflP_YA?t=25)
- [Что такое категории контента в HTML5?](https://youtu.be/rWEsjNWBoIE?t=105)
- [Какие категории считаются основными категориями контента?](https://youtu.be/rWEsjNWBoIE?t=167)
- [Разница между блочными и строчными элементами?](https://youtu.be/Uc378Fmc5vw?t=29)
- [Чем отличается `<article>` от `<section>`?](https://youtu.be/xIGp2FCxqj0?t=232)
- [Как семантически правильно сверстать картинку с подписью?](https://youtu.be/1eIRTdgzHtw?t=67)
- [Как семантически верно сверстать навигационное меню?](https://youtu.be/VYQl2GhbCUs?t=376)
- [Типы списков в HTML?](https://youtu.be/1eIRTdgzHtw?t=29)
- [Какая разница между тэгами `<strong>`, `<em>` и `<b>`, `<i>`?](https://youtu.be/ycYp7CYOnO0?t=202)
- [Для чего используются тэги `<sub>` и `<sup>`?](https://youtu.be/VYQl2GhbCUs?t=491)
- [Расскажите о тэге `<samp>`?](https://youtu.be/zcF-CVtXSBI?t=309)
- [Что такое мета-тэги?](https://youtu.be/ngyOYuTrUk8?t=205)
- [Что описывается в тэге `<head>`?](https://youtu.be/ngyOYuTrUk8?t=252)
- [Расскажите о meta-тэге с `name="viewport"`?](https://youtu.be/rWEsjNWBoIE?t=36)
- [Что такое инлайновый стиль? Можно ли его переопределить?](https://youtu.be/G7hLwudGWL4?t=155)
- [Есть ли у HTML элементов свои дефолтные специфичные стили?](https://youtu.be/G7hLwudGWL4?t=181)
- [Какой тэг использовать для того, чтобы сверстать кнопку?](https://youtu.be/G7hLwudGWL4?t=96)
- [Разница между кнопкой и ссылкой в HTML?](https://youtu.be/J6CgOSKFOlw?t=30)
- [Типы `<input>` элементов в HTML?](https://youtu.be/CjdCxxqObaM?t=87)
- [Для чего используется тэг `<label>`?](https://youtu.be/3NGkctg4lsE?t=338)
- [Для чего используется элемент `<datalist>`?](https://youtu.be/rlWgI7AvV18?t=178)
- [Как можно сгруппировать опции внутри тэга `<select>`?](https://youtu.be/trriSYNrHw4?t=304)
- [Основные атрибуты HTML-форм? Как они влияют на отправку данных с веб-страницы?](https://youtu.be/ia3VPHMESnY?t=262)
- [Опишите процесс валидации форм в HTML5?](https://youtu.be/NxPUaFAIyL0?t=26)
- [Для чего нужен атрибут `autocomplete`?](https://youtu.be/rWEsjNWBoIE?t=612)
- [Для чего используют атрибут `novalidate`?](https://youtu.be/trriSYNrHw4?t=451)
- [Для чего используется атрибут `pattern`?](https://youtu.be/xIGp2FCxqj0?t=111)
- [Для чего используют атрибут `inputmode`?](https://youtu.be/trriSYNrHw4?t=512)
- [Для чего используется атрибут `autofocus`? Где он применяется?](https://youtu.be/1kzbNflP_YA?t=128)
- [Для чего используется атрибут `enterkeyhint`?](https://youtu.be/J6CgOSKFOlw?t=256)
- [Для чего используется атрибут `capture`?](https://youtu.be/zcF-CVtXSBI?t=376)
- [Для чего используется атрибут `decoding`?](https://youtu.be/J6CgOSKFOlw?t=150)
- [Что такое элемент `<output>` в HTML5?](https://youtu.be/rWEsjNWBoIE?t=669)
- [Что такое свойство `valueAsNumber`?](https://youtu.be/rWEsjNWBoIE?t=717)
- [Для чего используются `data-`атрибуты?](https://youtu.be/rlWgI7AvV18?t=34)
- [Для чего используется атрибут `alt` и зачем он нужен?](https://youtu.be/G7hLwudGWL4?t=230)
- [Для чего используется элемент `<picture>`?](https://youtu.be/70VnuTXi4Wk?t=189)
- [Что такое `srcset`? Как работает `srcset`?](https://youtu.be/70VnuTXi4Wk?t=254)
- [Как можно изменить форму картинки или HTML элемента?](https://youtu.be/xIGp2FCxqj0?t=163)
- [Что такое `<canvas>`? И для чего он используется?](https://youtu.be/CjdCxxqObaM?t=200)
- [Что такое `<svg>` и `<canvas>`?](https://youtu.be/rWEsjNWBoIE?t=287)
- [Разница между `<canvas>` и `<svg>`?](https://youtu.be/rWEsjNWBoIE?t=340)
- [В каких случаях лучше использовать `<canvas>`, а в каких `<svg>`?](https://youtu.be/rWEsjNWBoIE?t=390)
- [Плюсы и минусы `<canvas>` и `<svg>`?](https://youtu.be/rWEsjNWBoIE?t=443)
- [Расскажите об особенностях стилизации `<svg>`?](https://youtu.be/xIGp2FCxqj0?t=298)
- [Для чего используются тэги `<tr>`, `<th>`, `<td>`?](https://youtu.be/ngyOYuTrUk8?t=306)
- [Какие HTML-элементы используются для создания и форматирования таблиц? Какие атрибуты у них есть?](https://youtu.be/ia3VPHMESnY?t=361)
- [Что такое `<iframe>`?](https://youtu.be/VYQl2GhbCUs?t=415)
- [Плюсы и минусы использования `<iframe>`?](https://youtu.be/NxPUaFAIyL0?t=120)
- [Чем отличается `<iframe>` от `<embed>`?](https://youtu.be/1kzbNflP_YA?t=297)
- [Для чего используется тэг `<template>`?](https://youtu.be/V39eEU9Pwv0?t=26)
- [Для чего используется тэг `<dialog>` в HTML?](https://youtu.be/1kzbNflP_YA?t=235)
- [Для чего используются `<noscript>` и `<script>` в HTML?](https://youtu.be/1kzbNflP_YA?t=174)
- [Как можно скрыть элемент разметки не используя CSS и JS?](https://youtu.be/VYQl2GhbCUs?t=539)
- [Разница между `<meter>` и `<progress>`?](https://youtu.be/VYQl2GhbCUs?t=608)
- [Что такое и зачем нужен атрибут `rel="nofollow"` в HTML?](https://youtu.be/3bC0orWHc5g?t=703)
- [Что такое атрибут `target`? Какие значения он принимает?](https://youtu.be/ovV8GhIkzBE?t=30)
- [Что такое ApplicationCache в HTML5?](https://youtu.be/ovV8GhIkzBE?t=92)
- [Способы улучшения производительности веб-страницы при использовании HTML?](https://youtu.be/ia3VPHMESnY?t=162)
- [Что такое поток документа?](https://youtu.be/trriSYNrHw4?t=102)
